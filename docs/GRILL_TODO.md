...
- [x] 런 예상 플레이타임/턴수 — 10~15분 짧은 런
- [x] 챕터별 난이도 곡선 — 스탯/규모/파트풀 상승 + 후반 제약형 모디파이어
- [x] 절차적 파트풀 구성 — 태그 기반 단일 풀
- [x] 보스/특수 인카운터 존재 여부 — 핵심 챕터에만 손수설계 고정 보스

### 레벨 — 경험

- [x] 레벨 디자인 (공간감/플레이 리듬/경험 흐름) — 초반 영지 질주 → 후반 충돌 격화 곡선

### 시나리오 테마

- [x] 시나리오 테마 목록 확정 — 9개(공성전/필드전/점령전/좀비전/레이스전/호위전/미궁전/대장전/개척전)
- [x] 테마별 규칙 차이 정의 — 테마마다 한 줄 규칙차이로 CONTEXT.md에 정의
- [x] 챕터별 테마 배정 방식 — 설계자가 챕터마다 직접 지정
