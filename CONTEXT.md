...
**Scenario Theme**:
A named ruleset variant a Run can be built around, layered on top of the base Faction/Dice Pool/Territory system rather than replacing it — this is how "every map/Chapter can differ" (see Grid, Part Pool) gets organized into recognizable, distinct play rhythms instead of just numeric variation. Nine themes: **Siege** (공성전, a fixed defended point vs. attackers), **Field** (필드전, the open engagement baseline — early land-grab into late-Run collisions, see Run), **Occupation** (점령전, Territory must be held for a duration to count, not claimed instantly), **Zombie** (좀비전, Wild Faction pressure is the dominant threat and grows over the Run), **Race** (레이스전, reaching a goal tile fast matters more than combat — closest to Dice of Afterimage's original board-race identity), **Escort** (호위전, protect a weak moving unit to a destination), **Labyrinth** (미궁전, built around concealment Terrain and random-teleport Event Tiles), **Commander Battle** (대장전, built around direct Master-vs-Master Contest rather than Natial-driven Skirmish), and **Frontier** (개척전, Natial summoning is disabled entirely — a Faction's first capture of each Territory tile grants one random Natial instead, so army-building and land-grabbing become the same act).
_Avoid_: Game mode, map type (Scenario Theme specifically means a rule variant, not a cosmetic/biome difference — see Terrain and 맵 비주얼 for that)
...
**Run**:
One roguelike playthrough of a single continuous grid map within a Chapter, ending in either completion or defeat, targeting a 10-15 minute playtime — short enough that a failure (no retry, see Chapter) doesn't cost much, and short enough to replay several times in one sitting. Intended pacing arc: an early land-grab (racing to claim open Territory while it's still unclaimed, since every other Faction is doing the same) that empties out into late-Run collisions (open space runs out, Factions increasingly bump into each other's Territory) — tension rises because the map fills up, not because of a scripted timer. The grid map doubles as both board (enemies, Territory, events, loot as tiles the Master moves across via Dice Pool movement) and battlefield (Contests happen in place, on the same grid, the instant a Master's move lands on a hostile tile) — there is no separate board screen and battle screen. Each Run is economically self-contained: Run GP and Territory both reset to nothing at the start of the next Run. The only thing carrying from one Run to the next within a Chapter is story progress — clearing a Run advances the Chapter's scenario trigger sequence toward the next one.
